...
int strrcasecmp(const char *s1, const char *s2)
{
   int off = (int) strlen(s1) - (int) strlen(s2); // typecast size_t to int because size_t typically is unsigned
    return stricmp(s1 + (off < 0 ? 0 : off), s2);
}
...
int main(int argc, char **argv)
{
   ...
   char *input = "Z:/tmp/source.jpg";
   char *output = "Z:/tmp/target.png";
   // test input file type and read file
   if (!strrcasecmp(input, ".png"))
   {
      // read PNG file
      sfc = cairo_image_surface_create_from_png(input);
   }
   else if (!strrcasecmp(input, ".jpg"))
   {
      // read JPEG file
      sfc = cairo_image_surface_create_from_jpeg(input);
   }
   else
   {
   ...
   if (!strrcasecmp(output, ".png"))
   {
       // write PNG file
      cairo_surface_write_to_png(sfc, output);
   }
   else if (!strrcasecmp(output, ".jpg"))
   {
      // write JPEG file
      cairo_image_surface_write_to_jpeg(sfc, output, 90);
   }
   else
   {
