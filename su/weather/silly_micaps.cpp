...
#define SIILY_MICAPS_SKIP_BLANK(s, off, idx)  \
off = idx;\
...
bool micaps_diamond_4::check_head(const std::string& content, size_t& offset)
{
    bool status = false;
    ...
    size_t i = offset;
    SIILY_MICAPS_SKIP_BLANK(content, offset, i);
...
bool micaps_diamond_4::is_valid() {
    bool status = false;
    // 
...
bool silly_diamond_4::read(const std::string &path, micaps_diamond_4 &md4) {
	bool status = false;
	std::string content;
...
bool silly_diamond_4::mmap_read(const std::string& path, micaps_diamond_4& md4)
{
	return false;
}

bool silly_diamond_4::write(const std::string& path, const micaps_diamond_4& md4)
{
    return false;
}
