...
class gpc
{
  public:
    ...
    void gpc_polyline_clip(gpc_op op, gpc_polygon *subj, gpc_polygon *clip, gpc_polygon *result);

    BOOL calc_intersection(double xa, double ya, double xb, double yb, double xc, double yc, double xd, double yd, double *x, double *y);
    ...
    edge_node *build_lmt(lmt_node **lmt, sb_tree **sbtree, int *sbt_entries, gpc_polygon *p, int type, gpc_op op)
    {
        ...
        MALLOCEDG(edge_table, total_vertices * sizeof(edge_node), "edge table creation");
    ...
    void add_intersection(it_node **it, edge_node *edge0, edge_node *edge1, double x, double y)
    {
    ...
    void add_st_edge(st_node **st, it_node **it, edge_node *edge, double dy)
    {
    ...
    void add_local_min(polygon_node **p, edge_node *edge, double x, double y)
    {
    ...
    void new_tristrip(polygon_node **tn, edge_node *edge, double x, double y)
    {
        if (!(*tn))
        {
    ...
    void minimax_test(gpc_polygon *subj, gpc_polygon *clip, gpc_op op)
    {
        ...
        MALLOCINT(o_table, subj->num_contours * clip->num_contours * sizeof(int), "overlap table creation");

        /* Check all subject contour bounding boxes against clip boxes */
        for (s = 0; s < subj->num_contours; s++)
            for (c = 0; c < clip->num_contours; c++)
                o_table[c * subj->num_contours + s] = (!((s_bbox[s].xmax < c_bbox[c].xmin) || (s_bbox[s].xmin > c_bbox[c].xmax))) && (!((s_bbox[s].ymax < c_bbox[c].ymin) || (s_bbox[s].ymin > c_bbox[c].ymax)));
