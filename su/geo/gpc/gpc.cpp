...
void gpc::gpc_read_polygon(FILE *fp, int read_hole_flags, gpc_polygon *p)
{
    ...
    MALLOCINT(p->hole, p->num_contours * sizeof(int), "hole flag array creation");
...
void gpc::gpc_write_polygon(FILE *fp, int write_hole_flags, gpc_polygon *p)
{
    ...
    for (c = 0; c < p->num_contours; c++)
    {
        ...
        for (v = 0; v < p->contour[c].num_vertices; v++)
            fprintf(fp, "% .*lf % .*lf\n", DBL_DIG, p->contour[c].vertex[v].x, DBL_DIG, p->contour[c].vertex[v].y);
    }
}
...
void gpc::gpc_polygon_clip(gpc_op op, gpc_polygon *subj, gpc_polygon *clip, gpc_polygon *result)
{
    ...
    if (result->num_contours > 0)
    {
        ...
        for (poly = out_poly; poly; poly = npoly)
        {
            ...
            if (poly->active)
            {
                ...
                MALLOCGPCV(result->contour[c].vertex, result->contour[c].num_vertices * sizeof(gpc_vertex), "vertex creation");
...
void gpc::gpc_tristrip_clip(gpc_op op, gpc_polygon *subj, gpc_polygon *clip, gpc_tristrip *result)
{
    sb_tree *sbtree = nullptr;
    it_node *it = nullptr, *intersect;
    edge_node *edge, *prev_edge, *next_edge, *succ_edge, *e0, *e1;
    ...
    {
        MALLOCGPC(result->strip, result->num_strips * sizeof(gpc_vertex_list), "tristrip list creation");
        ...
        for (tn = tlist; tn; tn = tnn)
        {
            ...
            if (tn->active > 2)
            {
                ...
                MALLOCGPCV(result->strip[s].vertex, tn->active * sizeof(gpc_vertex), "tristrip creation");
                v = 0;
                if (INVERT_TRISTRIPS)
                {
                    lt = tn->v[RIGHT];
...
/*         取得一条折线被一个多边形裁剪后的折线                */
/*-------------------------------------------------------------*/
void gpc::gpc_polyline_clip(gpc_op op, gpc_polygon *subj, gpc_polygon *clip, gpc_polygon *result)
{
...
double tt,t,S;
  ...
  if(t>=0 && t<=1 && S>=0 && S<=1){	//存在交点
                ...
                */
// 计算两条线段的交点
BOOL gpc::calc_intersection(double xa, double ya, double xb, double yb, double xc, double yc, double xd, double yd, double *x, double *y)
{
    double tt, t, S;
