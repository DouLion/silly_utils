...
class GeoPoint : public silly_utils::Double2
{
  public:
    ...
    GeoPoint(const double x, const double y) : Double2(x, y)
    {
...
class GeoRect : public silly_utils::Rectangle<double>
{
  public:
    ...
    bool IsValid()
    {
        return !(MathUtils::IsZero(g_left) && MathUtils::IsZero(g_top) && MathUtils::IsZero(g_right) && MathUtils::IsZero(g_bottom) && g_left <= g_right && g_top <= g_bottom);
    }
    ...
    {
        return (MathUtils::IsEqual(g_left, tmpGeoRect.g_left) && MathUtils::IsEqual(g_top, tmpGeoRect.g_top) && MathUtils::IsEqual(g_right, tmpGeoRect.g_right) && MathUtils::IsEqual(g_bottom, tmpGeoRect.g_bottom));
    }
};
