...
google_pyramid_block google_pyramid_util::get_block_rect(size_t l, size_t c, size_t r)
{
    ...
    ret_block.rect.min.x = c * x_delta + (0 - SILLY_GLOBAL_LEFT);  // 原点x方向上的偏移量
    ret_block.rect.max.x = ret_block.rect.min.x + x_delta;
    ret_block.rect.max.y = SILLY_GLOBAL_TOP - y_delta * r;  // 方向上由上到下, 与经纬度表示的相反, 并且减去y方向上的偏移量
    ret_block.rect.min.y = ret_block.rect.max.y - y_delta;
