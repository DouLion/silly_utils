...
int silly_minizip::compressZip(std::string src, std::string dst)
{
    ...
    if (S_ISREG(fileInfo.st_mode))  // 文件
    {
        ...
        if (ret != ZIP_OK)
        {
            ...
            zipClose(zFile, NULL);
            return SILLY_ZIP_ERROR_WRITE_NEWZIP;
        }
        zipClose(zFile, NULL);
...
int silly_minizip::decompressZip(const std::string& zipFileName, const std::string& outputFireDir)
{
    ...
    if (!std::filesystem::exists(outputDir))
    {
        if (!std::filesystem::create_directory(outputDir))
        {
            ...
            unzClose(zipFile);
            return SILLY_ZIP_ERROR_CREATE_DIR;
        }
    }
    // 进入ZIP文件中的第一个文件
    ...
    do
    {
        ...
        if (unzGetCurrentFileInfo(zipFile, &file_info, filename, sizeof(filename), nullptr, 0, nullptr, 0) != UNZ_OK)
        {
            ...
            status = SILLY_ZIP_ERROR_GET_INFO;
            break;
        }
        ...
        else       // 文件
        {
            ...
            if (!outFile.is_open())
            {
                ...
                status = SILLY_ZIP_ERROR_CREATE_FILE;
                break;
            }
            else
            {
...
#ifndef NDEBUG
    if (status == SILLY_ZIP_OK)
    {
        std::cout << "Decompression successful: " << zipFileName << std::endl;
    }
    else
    {
        std::cout << "Decompression :faile " << zipFileName << std::endl << "ERROR:" << status << std::endl;
