...
sliiy_warn_rule_record::sliiy_warn_rule_record(const std::string& atm, float intv, int grade, float swc, float drrp, float drp)
	: atm(atm), intv(intv), grade(grade), swc(swc), drrp(drrp), drp(drp)
{
}
