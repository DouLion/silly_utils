...
#define SILLY_PPTN_R_LENGHT SIILY_STCD_LENGHT + sizeof(unsigned int) * 2 + sizeof(float)
...
bool silly_pptn::serialize(unsigned char** c_in, size_t& len)
{
    ...
    memcpy(buff, stcd_buff, SIILY_STCD_LENGHT);
    ...
    memcpy(buff + 10, &ymd, sizeof(ymd));
    memcpy(buff + 10 + sizeof(ymd), &hms, sizeof(hms));
    memcpy(buff + 10 + sizeof(ymd) + sizeof(hms), &drp, sizeof(drp));
...
{
    if (len != SILLY_PPTN_R_LENGHT)
    {
        return false;
    }
    char* p = (char*)c_in;
    stcd.resize(SIILY_STCD_LENGHT);
    memcpy(&(stcd[0]), p, SIILY_STCD_LENGHT);
    unsigned int ymd = 0, hms = 0;
 
    memcpy(&ymd, p + 10, sizeof(ymd));
    memcpy(&hms, p + 10 + sizeof(ymd), sizeof(hms));
    memcpy(&drp, p + 10 + sizeof(ymd) + sizeof(hms), sizeof(drp));
