...
void gpc::gpc_read_polygon(FILE *fp, int read_hole_flags, gpc_polygon *p)
{
	...
	MALLOCGPC(p->contour, p->num_contours
	...
	for (c= 0; c < p->num_contours; c++)
	{
		...
		MALLOCGPCV(p->contour[c].vertex, p->contour[c].num_vertices
...
void gpc::gpc_add_contour(gpc_polygon *p, gpc_vertex_list *new_contour, int hole)
{
	...
	MALLOCINT(extended_hole, (p->num_contours + 1)
		* sizeof(int), "contour hole addition");
	
	/* Create an extended contour array */
	MALLOCGPC(extended_contour, (p->num_contours + 1)
	...
	MALLOCGPCV(extended_contour[c].vertex, new_contour->num_vertices
						   ...
						   gpc_polygon *result)
{
  ...
  {
	  MALLOCINT(result->hole, result->num_contours
		  * sizeof(int), "hole flag table creation");
	  MALLOCGPC(result->contour, result->num_contours
		  * sizeof(gpc_vertex_list), "contour creation");
...
BOOL gpc::inBox(double x, double y, double xc,double yc,double xd,double yd)
{
	if(xc>xd)swap(&xc,&xd);
	if(yc>yd)swap(&yc,&yd);
	if(x>=xc && x<=xd && y>=yc && y<=yd)return TRUE;
	else return FALSE;
}
