...
#pragma region Boost
	...
	{
#ifdef UNICODE
		std::wstring dirPathWStr = TZXCONVERT::Cxx11PlusS2WS(dirPath);
...
bool FileUtils::IsRegularFile(const char* filePath)
{
	...
	{
#ifdef UNICODE
	std::wstring filePathWStr = TZXCONVERT::Cxx11PlusS2WS(filePath);
...
bool FileUtils::ModifyLastWriteTime(const char* dstPath, const std::filesystem::file_time_type& time)
{
	...
	{
#ifdef UNICODE
	std::wstring dstPathWStr = TZXCONVERT::Cxx11PlusS2WS(dstPath);
...
#ifdef UNICODE
	std::wstring pathWStr = TZXCONVERT::Cxx11PlusS2WS(path);
	pathFile = pathWStr;
#else
	pathFile = path;
#endif
...
std::string FileUtils::GetParentDirectory(const char* currentPath)
{
	...
	{
#ifdef UNICODE
	std::wstring currentPathWStr = TZXCONVERT::Cxx11PlusS2WS(currentPath);
...
//		std::wstring filterWStr = TZXCONVERT::Cxx11PlusS2WS(filter);
...
void FileUtils::ListFiles(const char* currentPath, const std::string& filter, std::vector<std::string>& filePathVec)
{
	try
	{
...
#ifdef UNICODE
		wsregex reg = wsregex::compile(TZXCONVERT::Cxx11PlusS2WS(str));
		...
		{
			std::wstring tmpFileNameWStr = TZXCONVERT::Cxx11PlusS2WS(paths[i].filename().string());
...
#ifdef UNICODE
		std::wstring srcFilePathWStr = TZXCONVERT::Cxx11PlusS2WS(srcFilePath);
		std::filesystem::path srcPath(srcFilePathWStr);
		std::wstring dstFilePathWStr = TZXCONVERT::Cxx11PlusS2WS(dstFilePath);
...
#ifdef UNICODE
		std::wstring srcDirPathWStr = TZXCONVERT::Cxx11PlusS2WS(srcDirPath);
...
#ifdef UNICODE
		std::wstring dstDirPathWStr = TZXCONVERT::Cxx11PlusS2WS(dstDirPath);
...
#include <boost/progress.hpp>
		...
		BOOST_FOREACH(std::string& filePath, filePathVec)
		{
			...
			std::wstring filePathWStr = TZXCONVERT::Cxx11PlusS2WS(filePath);
			p = filePathWStr;
			std::wstring nameWStr = TZXCONVERT::Cxx11PlusS2WS(p.string().substr(srcPath.string().length()));
...
bool FileUtils::Rename(const char* srcPath, const char* dstPath)
{
	...
	{
#ifdef UNICODE
		std::wstring srcPathWStr = TZXCONVERT::Cxx11PlusS2WS(srcPath);
