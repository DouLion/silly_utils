...
std::vector<CalcResult> RainRetentionCapacity::CalcNYNL(CalcParam& p)
{
    ...
    for (int i = 0; i < pTargetRZ.size(); i++)
    {
        ...
        {
            // 反算
            // const {PP : rPPF.PP, PE : PE1, dW : dW1, OTQ : OTQ1, OTW : OTW1} = CalcPPF(BeginRZ, KCH, TCH, WCH, pRZ2WLine, pLine, Pa, Wm, Area, Bdgc, Yhdk, Yhdgc, EndRZ);
            auto rPPF = CalcPPF(p);
            ...
            // 第二次计算PP（使用相同的参数�?
            // const {PP : rPPZ.PP, PE : rPPZ.PE, dW : dW2, OTW : OTW2} = CalcPPZ(BeginRZ, KCH, pRZ2WLine, pLine, Pa, Wm, pmin, pmax, CalcSteps, Area, Bdgc, Yhdk, Yhdgc, EndRZ);
            auto rPPZ = CalcPPZ(p);
...
CalcResult RainRetentionCapacity::CalcPPZ(CalcParam& p) const
{
    ...
    CalcResult ret;
    // 计算最终降雨量PP
    ret.PP = pLine.GetP(p.Pa, PE, p.Wm);
...
CalcResult RainRetentionCapacity::CalcPPF(CalcParam& p)
{
    ...
    // 计算降雨量PP
    double PP = pLine.GetP(p.Pa, PE, p.Wm);
    CalcResult ret;
    ret.PE = std::round(PE * 100) / 100;
    ret.PP = std::round(PP * 100) / 100;
