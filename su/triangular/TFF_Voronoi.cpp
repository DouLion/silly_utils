	...
	for (; it != m_setTsaiKeyPts.end(); ++it)
	{
		...
		for (; it2 != m_setTsaiTriangles.end(); ++it2)
		{
			if (it2->GetVertex(0)->GetPoint() == it->GetPoint() ||
				it2->GetVertex(1)->GetPoint() == it->GetPoint() ||
				it2->GetVertex(2)->GetPoint() == it->GetPoint())
			{
				break;
			}
		}
		...
		do
		{
			...
			for (; it2_2 != m_setTsaiTriangles.end(); ++it2_2)
			{
				if ((it2_2->GetVertex(0)->GetPoint() == it->GetPoint() ||
					it2_2->GetVertex(1)->GetPoint() == it->GetPoint() ||
					it2_2->GetVertex(2)->GetPoint() == it->GetPoint()) &&
					(it2_2->GetVertex(0)->GetPoint() == unusedPt.GetPoint() ||
					it2_2->GetVertex(1)->GetPoint() == unusedPt.GetPoint() ||
					it2_2->GetVertex(2)->GetPoint() == unusedPt.GetPoint()) &&
					(it2_2->GetVertex(0)->GetPoint() != usedPt.GetPoint() &&
					it2_2->GetVertex(1)->GetPoint() != usedPt.GetPoint() &&
					it2_2->GetVertex(2)->GetPoint() != usedPt.GetPoint()))
				{
					bFind = true;
					usedPt = unusedPt;
					if (it2_2->GetVertex(0)->GetPoint() == it->GetPoint())
