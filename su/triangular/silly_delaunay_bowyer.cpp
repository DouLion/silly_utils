...
void silly_delaunay_bowyer::triangulate()
{
    ...
    for (auto const& pt : m_points)
    {
        ...
        m_tris.clear();
        std::vector<silly_dt_edge> invalid_edges;
        for (auto& tri : check_tris)
        {
            tri.calc_circle();
            if (tri.in_circle(pt))
            {
                invalid_edges.emplace_back(silly_dt_edge(tri.p0, tri.p1));
                invalid_edges.emplace_back(silly_dt_edge(tri.p1, tri.p2));
                invalid_edges.emplace_back(silly_dt_edge(tri.p2, tri.p0));
            }
            else
            {
...
void silly_delaunay_bowyer::init_super_triangle()
{
    ...
    m_tris.push_back(silly_dt_tri({xmin, ymin}, {xmax, ymin}, {xmax, ymax}));
    m_tris.push_back(silly_dt_tri({xmin, ymin}, {xmax, ymax}, {xmin, ymax}));
}
