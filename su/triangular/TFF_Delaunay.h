...
// 这个使用的是Lawson算法
/*
逐点插入的Lawson算法是Lawson在1977提出的，该算法思路简单，易于编程实现。基本原理为：
                首先建立一个大的三角形或多边形，把所有数据点包围起来，向其中插入一点，该点与包含它的三角形三个顶点相连，
        形成三个新的三角形，然后逐个对它们进行空外接圆检测，同时用Lawson设计的局部最优化过程LOP进行优化，即通
        过交换对角线的方法来保证所形成的三角网为Delaunay三角网。

        上述基于散点的构网算法理论严密、唯一性好，网格满足空圆特性，较为理想。由其逐点插入的构网过程可知，遇到非Delaunay边时，
通过删除调整，可以构造形成新的Delaunay边。在完成构网后，增加新点时，无需对所有点进行重新构网，只需要对新点的影响三角
形范围进行局部联网，且局部联网的方法简单易行。同样，点的删除、移动也可快速动态地进行。
        但在实际应用当中，这种构网算法当点集较大时网速度也较慢，如果点集范围是非凸区域或者存在内环，则会产生非法三角形。
...
struct PointFD
{
    ...
    PointFD operator+(const PointFD& p) const
    {
        return PointFD(X + p.X, Y + p.Y);
    }
    PointFD operator-(const PointFD& p) const
    {
        return PointFD(X - p.X, Y - p.Y);
...
class vertex
{
  public:
    ...
    vertex(double x, double y, double z) : m_Pnt(x, y, z)
    {
        value = z;
    }
    ...
    const silly_utils::Point& GetPoint() const
    {
        return m_Pnt;
    }
...
#define MIN_3(a, b, c) (a < b ? (a < c ? a : c) : (b < c ? b : c))
  ...
  public:
    ...
    {
        if ((x0 > OtherRect.x1) || (y0 > OtherRect.y1) || (x1 < OtherRect.x0) || (y1 < OtherRect.y0))
        {
            return false;
        }
        else
...
class triangle
{
  public:
    ...
    triangle(const triangle& tri) : m_Center(tri.m_Center), m_R(tri.m_R), m_R2(tri.m_R2), m_Rect(tri.m_Rect)
    {
        for (int i = 0; i < 3; i++)
