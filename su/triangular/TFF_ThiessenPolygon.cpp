...
ThiessenPolygon::~ThiessenPolygon(void) = default;
...
bool ThiessenPolygon::IsPointOnPoly(const vertex& ver) const
{
    ...
    for (i = 0, j = pointNum - 1; i < pointNum; j = i++)
    {
        ...
        if ((((vecVer.at(i).GetY() <= y) && (y < vecVer.at(j).GetY()) || ((vecVer.at(j).GetY() <= y) && (y < vecVer.at(i).GetY()))) &&
...
ThiessenPolygon::PositionalRelationship ThiessenPolygon::JudgePositionalRelationship(const ThiessenPolygon& poly)
{
    ...
    for (size_t i = 0; i < nPolyPtsA; ++i)
    {
        ...
        {
            if (1 == calc.seg_cross(this->vecPolygonPts[i], this->vecPolygonPts[(i + 1) % nPolyPtsA], poly.vecPolygonPts[j], poly.vecPolygonPts[(j + 1) % nPolyPtsB], vP))
            {
