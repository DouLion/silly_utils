 ...
 *
 * @brief silly::thread_pool: a fast, lightweight, and easy-to-use C++17 thread pool library. This header file contains the main thread pool class and some additional classes and definitions. No other files are needed in order to use the thread pool itself.
 */
 ...
 */
    namespace this_thread {
        ...
        public:
            /**
             * @brief Get the index of the current thread. If this thread belongs to a `silly::thread_pool` object, it will have an index from 0 to `silly::thread_pool::get_thread_count() - 1`. Otherwise, for example if this thread is the main thread or an independent `std::thread`, `std::nullopt` will be returned.
             *
        ...
        public:
            /**
             * @brief Get the pointer to the thread pool that owns the current thread. If this thread belongs to a `silly::thread_pool` object, a pointer to that object will be returned. Otherwise, for example if this thread is the main thread or an independent `std::thread`, `std::nullopt` will be returned.
             *
...
#define SILLY_THREAD_POOL_PAUSED_OR_EMPTY tasks.empty()
        ...
        void wait()
        {
            ...
            tasks_done_cv.wait(tasks_lock,
                               [this]
                               {
                                   return (tasks_running == 0) && SILLY_THREAD_POOL_PAUSED_OR_EMPTY;
                               });
            waiting = false;
        }
        ...
        void worker(const concurrency_t idx, const std::function<void()>& init_task)
        {
            ...
            while (true)
            {
                ...
                task_available_cv.wait(tasks_lock,
                                       [this]
                                       {
                                           return !SILLY_THREAD_POOL_PAUSED_OR_EMPTY || !workers_running;
                                       });
                if (!workers_running)
                    break;
                {
