...
Float4::Float4(float x, float y, float z, float w) : m_x(x), m_y(y), m_z(z), m_w(w)
{
}

Float4::Float4(const Float4& rhv) : m_x(rhv.m_x), m_y(rhv.m_y), m_z(rhv.m_z), m_w(rhv.m_w)
{
}
...
{
    cmpresult res = 0;
    if (v0.m_x <= v1.m_x + epsilon.m_x && v1.m_x <= v0.m_x + epsilon.m_x)
        res |= (1 << 0);
    if (v0.m_y <= v1.m_y + epsilon.m_y && v1.m_y <= v0.m_y + epsilon.m_y)
        res |= (1 << 1);
    if (v0.m_z <= v1.m_z + epsilon.m_z && v1.m_z <= v0.m_z + epsilon.m_z)
        res |= (1 << 2);
    return res;
}
...
Float4::cmpresult Float4::NearEqual(const Float4& v0, const Float4& v1, const Float4& epsilon)
{
    ...
    if (v0.m_z <= v1.m_z + epsilon.m_z && v1.m_z <= v0.m_z + epsilon.m_z)
        res |= (1 << 2);
    if (v0.m_w <= v1.m_w + epsilon.m_w && v1.m_w <= v0.m_w + epsilon.m_w)
        res |= (1 << 3);
    return res;
}
