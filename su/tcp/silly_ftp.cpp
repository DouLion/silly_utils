...
int silly_ftp::socket_wait(ftphandle* ctl)
{
	...
	if (ctl->dir == silly_ftp_WRITE) wfd = &fd;
	else rfd = &fd;
...
#ifndef NOSSL
				if (nData->tlsctrl) w = SSL_write(nData->ssl, nbp, silly_ftp_BUFSIZ);
				else w = net_write(nData->handle, nbp, silly_ftp_BUFSIZ);
#else
...
{
	if ((mp_ftphandle) && (mp_ftphandle->dir == silly_ftp_CONTROL)) return mp_ftphandle->response;
	return NULL;
}
...
int silly_ftp::FtpSendCmd(const char* cmd, char expresp, ftphandle* nControl)
{
	...
	if (nControl->dir != silly_ftp_CONTROL) return 0;
	sprintf(buf, "%s\r\n", cmd);
...
int silly_ftp::FtpAccess(const char* path, accesstype type, transfermode mode, ftphandle* nControl, ftphandle** nData)
{
	...
	if ((path == NULL) && ((type == silly_ftp::filewrite)
		|| (type == silly_ftp::fileread)
		|| (type == silly_ftp::filereadappend)
		|| (type == silly_ftp::filewriteappend)))
	{
...
int silly_ftp::FtpOpenPort(ftphandle* nControl, ftphandle** nData, transfermode mode, int dir, char* cmd)
{
	...
	if (nControl->dir != silly_ftp_CONTROL) return -1;
...
int silly_ftp::FtpClose(ftphandle* nData)
{
	...
	else if (nData->dir != silly_ftp_READ) return 0;
...
int silly_ftp::FtpWrite(void* buf, int len, ftphandle* nData)
{
	...
	if (nData->dir != silly_ftp_WRITE) return 0;
...
int silly_ftp::FtpXfer(const char* localfile, const char* path, ftphandle* nControl, accesstype type, transfermode mode)
{
	...
	{
		char ac[3] = "  ";
		if ((type == silly_ftp::dir) || (type == silly_ftp::dirverbose)) { ac[0] = 'w'; ac[1] = '\0'; }
		if (type == silly_ftp::fileread) { ac[0] = 'w'; ac[1] = '\0'; }
		if (type == silly_ftp::filewriteappend) { ac[0] = 'r'; ac[1] = '\0'; }
		if (type == silly_ftp::filereadappend) { ac[0] = 'a'; ac[1] = '\0'; }
		if (type == silly_ftp::filewrite) { ac[0] = 'r'; ac[1] = '\0'; }
		if (mode == silly_ftp::image) ac[1] = 'b';
		...
		if (type == silly_ftp::filewriteappend) fseeko64(local, mp_ftphandle->offset, SEEK_SET);
	}
	if (local == NULL) local = ((type == silly_ftp::filewrite)
		|| (type == silly_ftp::filewriteappend)) ? stdin : stdout;
...
int silly_ftp::Get(const char* outputfile, const char* path, transfermode mode, off64_t offset)
{
	...
	if (offset == 0) return FtpXfer(outputfile, path, mp_ftphandle, silly_ftp::fileread, mode);
	else return FtpXfer(outputfile, path, mp_ftphandle, silly_ftp::filereadappend, mode);
}
...
{
	mp_ftphandle->offset = offset;
	if (offset == 0) return FtpXfer(inputfile, path, mp_ftphandle, silly_ftp::filewrite, mode);
	else return FtpXfer(inputfile, path, mp_ftphandle, silly_ftp::filewriteappend, mode);
}
...
int silly_ftp::Quit()
{
	if (mp_ftphandle->dir != silly_ftp_CONTROL) return 0;
