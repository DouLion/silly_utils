...
*/
LOGURU_EXPORT
void add_callback(const char* id, log_handler_t callback, void* user_data, Verbosity verbosity, close_handler_t on_close = nullptr, flush_handler_t on_flush = nullptr);
...
class LOGURU_EXPORT LogScopeRAII
{
  public:
    ...
    // issue #43
    LogScopeRAII(LogScopeRAII&& other) : _verbosity(other._verbosity), _file(other._file), _line(other._line), _indent_stderr(other._indent_stderr), _start_time_ns(other._start_time_ns)
    {
...
template <typename... Args>
LOGURU_EXPORT LOGURU_NORETURN void log_and_abort(int stack_trace_skip, const char* expr, const char* file, unsigned line, LOGURU_FORMAT_STRING_TYPE format, const Args&... args)
{
...
class EcEntryData : public EcEntryBase
{
  public:
    ...
    EcEntryData(const char* file, unsigned line, const char* descr, T data, Printer&& printer) : EcEntryBase(file, line, descr), _data(data), _printer(printer)
    {
    }
...
#define ERROR_CONTEXT(descr, data)                                                                   \
    const loguru::EcEntryData<loguru::make_ec_type<decltype(data)>::type> LOGURU_ANONYMOUS_VARIABLE( \
        error_context_scope_)(__FILE__, __LINE__, descr, data, static_cast<loguru::EcEntryData<loguru::make_ec_type<decltype(data)>::type>::Printer>(loguru::ec_to_text))  // For better error messages
...
#define VLOG_F(verbosity, ...) ((verbosity) > loguru::current_verbosity_cutoff()) ? (void)0 : loguru::log(verbosity, __FILE__, __LINE__, __VA_ARGS__)

// LOG_F(INFO, "Foo: %d", some_number);
#define LOG_F(verbosity_name, ...) VLOG_F(loguru::Verbosity_##verbosity_name, __VA_ARGS__)

#define VLOG_IF_F(verbosity, cond, ...) ((verbosity) > loguru::current_verbosity_cutoff() || (cond) == false) ? (void)0 : loguru::log(verbosity, __FILE__, __LINE__, __VA_ARGS__)

#define LOG_IF_F(verbosity_name, cond, ...) VLOG_IF_F(loguru::Verbosity_##verbosity_name, cond, __VA_ARGS__)

#define VLOG_SCOPE_F(verbosity, ...) loguru::LogScopeRAII LOGURU_ANONYMOUS_VARIABLE(error_context_RAII_) = ((verbosity) > loguru::current_verbosity_cutoff()) ? loguru::LogScopeRAII() : loguru::LogScopeRAII(verbosity, __FILE__, __LINE__, __VA_ARGS__)

// Raw logging - no preamble, no indentation. Slightly faster than full logging.
#define RAW_VLOG_F(verbosity, ...) ((verbosity) > loguru::current_verbosity_cutoff()) ? (void)0 : loguru::raw_log(verbosity, __FILE__, __LINE__, __VA_ARGS__)
...
#define CHECK_WITH_INFO_F(test, info, ...) LOGURU_PREDICT_TRUE((test) == true) ? (void)0 : loguru::log_and_abort(0, "CHECK FAILED:  " info "  ", __FILE__, __LINE__, ##__VA_ARGS__)
...
#define CHECK_OP_F(expr_left, expr_right, op, ...)                                                                                                                                                                                                          \
    ...
    {                                                                                                                                                                                                                                                       \
        ...
        {                                                                                                                                                                                                                                                   \
            ...
            auto fail_info = loguru::textprintf("CHECK FAILED:  " LOGURU_FMT(s) " " LOGURU_FMT(s) " " LOGURU_FMT(s) "  (" LOGURU_FMT(s) " " LOGURU_FMT(s) " " LOGURU_FMT(s) ")  ", #expr_left, #op, #expr_right, str_left.c_str(), #op, str_right.c_str()); \
...
#define VLOG_IF_S(verbosity, cond) ((verbosity) > loguru::current_verbosity_cutoff() || (cond) == false) ? (void)0 : loguru::Voidify() & loguru::StreamLogger(verbosity, __FILE__, __LINE__)
...
#define CHECK_OP_S(function_name, expr1, op, expr2)                                                                                                           \
    while (auto error_string = loguru::function_name(#expr1 " " #op " " #expr2, loguru::referenceable_value(expr1), #op, loguru::referenceable_value(expr2))) \
...
#else
   // Debug logging disabled:
#define DVLOG_IF_S(verbosity, cond) (true || (verbosity) > loguru::current_verbosity_cutoff() || (cond) == false) ? (void)0 : loguru::Voidify() & loguru::StreamLogger(verbosity, __FILE__, __LINE__)
