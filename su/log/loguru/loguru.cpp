...
#define ENABLE_VIRTUAL_TERMINAL_PROCESSING 0x0004
    ...
    {
        return 0 == strcmp(term, "cygwin") || 0 == strcmp(term, "linux") || 0 == strcmp(term, "rxvt-unicode-256color") || 0 == strcmp(term, "screen") || 0 == strcmp(term, "screen-256color") || 0 == strcmp(term, "screen.xterm-256color") ||
               0 == strcmp(term, "tmux-256color") || 0 == strcmp(term, "xterm") || 0 == strcmp(term, "xterm-256color") || 0 == strcmp(term, "xterm-termite") || 0 == strcmp(term, "xterm-color");
    }
    else
    {
        return false;
    ...
    for (int arg_it = 1; arg_it < argc; ++arg_it)
    {
        ...
        if (strncmp(cmd, verbosity_flag, arg_len) == 0 && !std::isalpha(cmd[arg_len], std::locale("")))
        {
            ...
            else
            {
                ...
                CHECK_F(end && *end == '\0', "Invalid verbosity. Expected integer, INFO, WARNING, ERROR or OFF, got '" LOGURU_FMT(s) "'", value_str);
            }
        }
        else
        {
...
void write_date_time(char* buff, size_t buff_size)
{
    ...
    snprintf(buff, buff_size, "%04d%02d%02d_%02d%02d%02d", 1900 + time_info.tm_year, 1 + time_info.tm_mon, time_info.tm_mday, time_info.tm_hour, time_info.tm_min, time_info.tm_sec);
}
...
void add_callback(const char* id, log_handler_t callback, void* user_data, Verbosity verbosity, close_handler_t on_close, flush_handler_t on_flush)
{
...
static void print_preamble(char* out_buff, size_t out_buff_size, Verbosity verbosity, const char* file, unsigned line)
{
    ...
    if (g_preamble_date && pos < out_buff_size)
    {
        pos += snprintf(out_buff + pos, out_buff_size - pos, "%02d-%02d-%02d ", 1900 + time_info.tm_year, 1 + time_info.tm_mon, time_info.tm_mday);
    }
    if (g_preamble_time && pos < out_buff_size)
    {
        pos += snprintf(out_buff + pos, out_buff_size - pos, "%02d:%02d:%02d||", time_info.tm_hour, time_info.tm_min, time_info.tm_sec);
    }
    /*if (g_preamble_uptime && pos < out_buff_size) {
...
static void log_message(int stack_trace_skip, Message& message, bool with_indentation, bool abort_if_fatal)
{
    ...
    if (verbosity <= g_stderr_verbosity)
    {
        if (g_colorlogtostderr && s_terminal_has_color)
        {
            ...
            {
                fprintf(stderr, "%s%s%s%s%s%s%s\n", terminal_reset(), verbosity == Verbosity_WARNING ? terminal_yellow() : terminal_red(), message.preamble, message.indentation, message.prefix, message.message, terminal_reset());
            }
        }
        else
        {
...
// stack_trace_skip is just if verbosity == FATAL.
void log_to_everywhere(int stack_trace_skip, Verbosity verbosity, const char* file, unsigned line, const char* prefix, const char* buff)
{
...
#if LOGURU_USE_FMTLIB
            auto prefix = textprintf("[ErrorContext] {.{}s}:{:-5u} {:-20s} ", filename(entry->_file), LOGURU_FILENAME_WIDTH, entry->_line, description.c_str());
#else
            auto prefix = textprintf("[ErrorContext] %*s:%-5u %-20s ", LOGURU_FILENAME_WIDTH, filename(entry->_file), entry->_line, description.c_str());
#endif
...
void install_signal_handlers(bool unsafe_signal_handler)
{
    ...
    for (const auto& s : ALL_SIGNALS)
    {
        CHECK_F(sigaction(s.number, &sig_action, NULL) != -1, "Failed to install handler for " LOGURU_FMT(s) "", s.name);
    }
}
}  // namespace loguru
