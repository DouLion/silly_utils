    ...
    if (conn.empty() || rebuild)
    {
        ...
        switch (type)  // 使用ODBC连接串
        {
            case enum_database_type::dbMYSQL:
                conn = "Driver={" + driver + "};Server=" + ip + ";Port=" + std::to_string(port) + ";Database=" + schema + ";User=" + user + ";Password=" + password + ";Option=3;charset=UTF8;";
                break;
            case enum_database_type::dbSQLSERVER:
                conn = "Driver={" + driver + "};Server=" + ip + ";Port:" + std::to_string(port) + ";Database=" + schema + ";UID=" + user + ";PWD=" + password + ";";
                break;
            case enum_database_type::dbORACLE:
                conn = "Driver={" + driver + "};DBQ=" + ip + ":" + std::to_string(port) + "/" + schema + ";Uid=" + user + ";Pwd=" + password + ";";
                break;
            default:
                break;
        }
