...
class depict
{
  ...
  public:
    ...
    /// <returns></returns>
    bool process(const suLine& cs0, const suLine& cs1, suLine& vsection, options& opt);
    ...
    /// <returns></returns>
    std::vector<std::vector<double>> process(const std::vector<suLine> csections, std::vector<suLine>& vsections, options& opt);
