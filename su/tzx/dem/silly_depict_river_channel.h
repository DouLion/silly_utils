...
class depict
{
  ...
  public:
    ...
    /// <returns></returns>
    bool process(const suLineZ& cs0, const suLineZ& cs1, suLineZ& vsection, options& opt);
    ...
    /// <returns></returns>
    std::vector<std::vector<double>> process(const std::vector<suLineZ> csections, std::vector<suLineZ>& vsections, options& opt);
