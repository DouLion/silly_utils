...
bool silly_pyramid::open(const std::string& root, const silly_mmap::open_mode& mode, bool usemmap)
{
    ...
    if (silly_mmap::open_mode::READONLY == mode) // 
    {
        ...
        if (!m_info.open(info_path.string().c_str(), silly_mmap::open_mode::READONLY, usemmap))
        {
            m_info.close();
            return false;
        }

        if (!m_data.m_index.open(index_path.string().c_str(), silly_mmap::open_mode::READONLY, usemmap))
        {
            m_info.close();
        ...
        if (!m_data.open(data_path.string().c_str(), silly_mmap::open_mode::READONLY, usemmap))
        {
            m_info.close();
            m_data.m_index.close();
            m_data.close();
