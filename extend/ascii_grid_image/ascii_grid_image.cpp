...
void create_map(double ncols, double nrows, double xllcorner, double yllcorner, double cellsize, double mid)
{
    if (!png_index)
    {
        ...
        png_index = new size_t[nrows * ncols];
        memset(png_index, 0, sizeof(size_t) * nrows * ncols);
        for (size_t r = 0; r < nrows - 2; ++r)
        {
            for (size_t c = 0; c < ncols - 2; ++c)
            {
                ...
                int gcol = static_cast<size_t>(std::round((tmp_gauss_x - gauss_left) / cellsize + 1));
                int grol = static_cast<size_t>(std::round((gauss_top - tmp_gauss_y) / cellsize));
                if (gcol >= 0 && grol > 0 && gcol < ncols && grol < nrows)
                {
                    size_t dpos = SU_MAX(0, SU_MIN(grol * ncols + gcol, nrows * ncols));
                    size_t spos = r * ncols + c;
                    png_index[spos] = static_cast<size_t>(dpos);
                }
            }
        }
    }
